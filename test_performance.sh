...
TIME="\t%e,\t%M" /usr/bin/time $TIMEV ${CURDIR}/../build$BUILD_TYPE/dbms/src/Server/clickhouse-performance-test --port `$extract --key=tcp_port` --r ${CURDIR}/../dbms/tests/performance $* > ${CURDIR}/performance${BUILD_TYPE}.`hostname`.`date '+%Y%M%''d%H%M%''S'`.json

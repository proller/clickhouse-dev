...
./generate_and_test.py --client="${pwd}/../build$BUILD_TYPE/dbms/src/Server/clickhouse-client" --port=`${pwd}/../build$BUILD_TYPE/dbms/src/Server/clickhouse-extract-from-config --config-file=${pwd}/config.xml --key=tcp_port` --no_break --no_mongo --no_mysql $*
